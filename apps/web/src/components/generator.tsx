...
// Mirrors the detail_kind enum in packages/database/src/schema.ts.
...
export function Generator({ onSubmit }: Props) {
  ...
  const [imageModels, setImageModels] = useState<CatalogImageModel[]>([]);
  ...
  const [detailReferences, setDetailReferences] = useState<Array<{ assetId: string; kind: string; previewUrl: string; filename: string }>>([]);
  ...
  // Empty selection means "use the server's default set for this count" —
  // picking specific angles is opt-in, not required. Whatever the customer
  // has picked (or not) is resolved through the exact same `resolveAngleSet`
  // the worker uses, so the preview here can never drift from what the
  // server actually generates — including its top-up behavior when fewer
  // angles are picked than the requested count.
  const [selectedAngles, setSelectedAngles] = useState<CameraAngleKey[]>([]);
  const ANGLE_OPTIONS = Object.values(CAMERA_ANGLES);
  const effectiveAngles = resolveAngleSet(imageCount, selectedAngles);
  const angleLabels = effectiveAngles.map((key) => CAMERA_ANGLES[key].label);

  function toggleAngle(key: CameraAngleKey) {
    setSelectedAngles((current) => {
      if (current.includes(key)) return current.filter((candidate) => candidate !== key);
      if (current.length >= imageCount) return current;
      return [...current, key];
    });
  }

  useEffect(() => {
    setSelectedAngles((current) => (current.length > imageCount ? current.slice(0, imageCount) : current));
  }, [imageCount]);
  ...
  async function handleDetailFile(file?: File) {
    if (!file || detailReferences.length >= 5) return;
    setMessage(null);
    try {
      const presign = await apiFetch<{ assetId: string; uploadUrl: string; headers: Record<string, string> }>("/api/uploads", {
        method: "POST",
        body: JSON.stringify({ filename: file.name, contentType: file.type, kind: "detail_reference" }),
      });
      const upload = await fetch(presign.uploadUrl, { method: "PUT", headers: presign.headers, body: file });
      if (!upload.ok) throw new Error("Detail reference upload failed");
      await apiFetch(`/api/uploads/${presign.assetId}/complete`, { method: "POST", body: JSON.stringify({ filename: file.name }) });
      setDetailReferences((current) => [...current, { assetId: presign.assetId, kind: "other", previewUrl: URL.createObjectURL(file), filename: file.name }]);
      setMessage("Detail reference uploaded and checked");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Detail reference upload failed");
    }
  }

  function setDetailKind(assetId: string, kind: string) {
    setDetailReferences((current) => current.map((d) => (d.assetId === assetId ? { ...d, kind } : d)));
  }

  function removeDetailReference(assetId: string) {
    setDetailReferences((current) => current.filter((d) => d.assetId !== assetId));
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const requestBody = { mainGarmentAssetId: assetId, detailReferences: detailReferences.map((d) => ({ assetId: d.assetId, kind: d.kind })), characterId: customCharacterAssetId ? null : selectedCharacterId, characterAssetId: customCharacterAssetId, environmentPresetId: environmentId, imageModelId: selectedImageModelId, resolution: resolution.toLowerCase(), aspectRatio: aspect.toLowerCase(), outputCount: Number.parseInt(count, 10), cameraAngles: effectiveAngles, inputType: "photo", ageAppearance: age, heightAppearance: height, pose };
  ...
  useEffect(() => {
    ...
    apiFetch<CatalogImageModel[]>("/api/customer/models").then((data) => { setImageModels(data); setSelectedImageModelId(data[0]?.id ?? null); }).catch(() => undefined);
    ...
    <div className="dashboard-grid">
      <section className="panel generator">
        ...
        <div className="form-body">
          <div className="form-section"><div className="section-head"><h3 className="section-heading">Start with a reference</h3><span className="section-caption">Required · 1 main image</span></div><label className="dropzone">{image ? <img className="upload-preview" src={image} alt="Uploaded garment reference" /> : <><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => handleFile(e.target.files?.[0])} /><div><span className="upload-symbol"><Icon name="plus" /></span><p className="drop-title">Drop your garment reference here</p><p className="drop-subtitle">JPG, PNG or WEBP · up to 25 MB</p></div></>}</label>
            <div className="detail-refs"><div className="detail-refs-head"><span>Detail references <span className="help">optional — close-ups of a border, embroidery, or anything easy to miss</span></span></div><div className="detail-refs-list">{detailReferences.map((d) => <div className="detail-ref-card" key={d.assetId}><img src={d.previewUrl} alt={d.filename} /><select value={d.kind} onChange={(e) => setDetailKind(d.assetId, e.target.value)}>{DETAIL_KIND_OPTIONS.map((option) => <option value={option.key} key={option.key}>{option.label}</option>)}</select><button type="button" className="detail-ref-remove" aria-label={`Remove ${d.filename}`} onClick={() => removeDetailReference(d.assetId)}>×</button></div>)}{detailReferences.length < 5 && <button type="button" className="detail-ref-add" onClick={() => detailFileInputRef.current?.click()}><Icon name="plus" /><span>Add</span></button>}<input ref={detailFileInputRef} type="file" accept="image/png,image/jpeg,image/webp" style={{display: "none"}} onChange={(e) => { handleDetailFile(e.target.files?.[0]); if (detailFileInputRef.current) detailFileInputRef.current.value = ""; }} /></div></div>
          </div>
          ...
          <div className="form-section"><div className="section-head"><h3 className="section-heading">Output</h3><span className="section-caption">{imageModels.length > 1 ? "Choose your model, or let the pipeline pick" : "The pipeline will pick the right model"}</span></div><div className="select-row"><div className="select-field"><label>Resolution</label><select value={resolution} onChange={(e) => setResolution(e.target.value)}><option>1K</option><option>2K</option><option>4K</option></select></div><div className="select-field"><label>Frame</label><select value={aspect} onChange={(e) => setAspect(e.target.value)}><option>Portrait</option><option>Square</option><option>Landscape</option></select></div><div className="select-field"><label>Set size</label><select value={count} onChange={(e) => setCount(e.target.value)}><option>1 image</option><option>2 images</option><option>3 images</option><option>4 images</option><option>5 images</option></select></div>{imageModels.length > 0 && <div className="select-field"><label>Image model</label><select value={selectedImageModelId ?? ""} onChange={(e) => setSelectedImageModelId(e.target.value || null)}>{imageModels.map((model) => <option value={model.id} key={model.id}>{model.name}</option>)}</select></div>}</div>{imageCount > 1 && <div className="angle-picker"><label>Camera angles <span className="help">Pick up to {imageCount}, or leave blank for our default set</span></label><div className="angle-picker-chips">{ANGLE_OPTIONS.map((option) => { const picked = selectedAngles.includes(option.key); const atCap = !picked && selectedAngles.length >= imageCount && selectedAngles.length > 0; return <button type="button" key={option.key} className={`angle-chip ${picked ? "selected" : ""}`} disabled={atCap} onClick={() => toggleAngle(option.key)}>{option.label}</button>; })}{selectedAngles.length > 0 && <button type="button" className="angle-chip angle-chip-clear" onClick={() => setSelectedAngles([])}>Reset to default</button>}</div></div>}</div>

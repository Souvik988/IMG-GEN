...
type RecentProject = { id: string; state: string; display: string; resolution: string; aspectRatio: string; createdAt: string };
...
export function Generator({ onSubmit }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      const requestBody = { mainGarmentAssetId: assetId, characterId: selectedCharacterId, environmentPresetId: environmentId, resolution: resolution.toLowerCase(), aspectRatio: aspect.toLowerCase(), outputCount: Number.parseInt(count, 10), inputType: "photo", ageAppearance: age, heightAppearance: height, pose };
